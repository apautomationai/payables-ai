...
const IntegrationLogo = ({ name }: { name: string }) => {
  ...
  if (name === "QuickBooks")
    return <BookUser className="w-8 h-8 text-green-600" />;
  return null;
};
          ...
          <div className="space-y-2.5 text-sm text-muted-foreground border-l-2 pl-4 py-2">
            {createdAt && (
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-primary">
                  {format(new Date(createdAt), dateTimeFormat)}
                </span>
              </div>
            )}
            ...
            {name.toLowerCase() === "gmail" && lastRead && (
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-primary">
                  {format(new Date(lastRead), dateTimeFormat)}
                </span>
              </div>
            )}
          </div>
              ...
              {allowCollection ? (
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>
                        This integration is not enabled for your account yet.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the date from which to start processing data. This can be
            changed later.
          </DialogDescription>
        </DialogHeader>
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            This will permanently disconnect your {backendName} account. We will
            stop processing any new data. You can reconnect at any time.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
